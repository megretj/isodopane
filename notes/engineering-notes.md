...
This project (including these notes) was nearly entirely vibe-coded using Anthropic Opus 5.

Findings from building isodapane — the data quirks, the modelling mistakes that
had to be corrected, and the measurements behind the numbers quoted in the
[README](../README.md). Recorded rather than summarised, because everything
downstream keys off them.

## Contents

- [Discovered data facts](#discovered-data-facts)
- [The WFS does not reproject](#the-wfs-does-not-reproject)
- [14 zone features carry no zone number](#14-zone-features-carry-no-zone-number)
- [The fare model, and where the original design was wrong](#the-fare-model-and-where-the-original-design-was-wrong)
...
dependency, [`crs.rs`](../crates/build/src/crs.rs) implements swisstopo's
published approximate LV95→WGS84 formula in about forty lines. Measured against
swisstopo's own reference point (Zimmerwald observatory) it lands **0.24 m** off
— two orders of magnitude finer than the ~15 m tolerance the polygons are then
simplified with, so it is exact for this purpose. A unit test pins that.
...
(see [`search.rs`](../crates/build/src/search.rs) for why one label per node
loses the optimum).
...
A `shapes.txt` from another feed is the exact fix if it is ever needed.
...
[`data/fares.toml`](../data/fares.toml) — derived independently of the search,
not read back out of it. Each fixture records its zone chain so the derivation is
auditable.
...
> These six prices are derived from the published zone plan and tariff rules,
> not read off a ZVV sales channel. If one ever disagrees with the ZVV app, the
> bug is in the graph or the zone assignment — never adjust the price table to
> make a fixture pass.
...
So the originally suggested cap of 16 was genuinely lossy. **The shipped cap is
64**: exact on every measured pair, with 2× margin over the smallest exact cap
(32) and comfortably under the 131 the frontier actually reaches. `validate`
reports the "priced differently" count on every run; a non-zero value means fares
have become upper bounds and needs investigating.
...
so the top of the slider always means "everywhere you can get to". A budget above
a lower ceiling is remembered rather than discarded, so toggling to the reduced
column and back does not quietly shrink it.
...
### The Rust and JavaScript searches are diffed

The JS search in [`web/app.js`](../web/app.js) is a direct port of
[`search.rs`](../crates/build/src/search.rs). They are checked against each other
with `validate --dump`, which writes per-origin results the JS is diffed against:
**0 of 1,269,600 station pairs differ**. The browser copy adds route
reconstruction, which the build side does not need; the fare logic is identical.
Keep them in step; if the duplication becomes annoying, compile the Rust to WASM
and delete the JS.
...
the **actual price** by class and reduction card — the price table and the tariff
kilometres together. See [`data/national/PROVENANCE.md`](../data/national/PROVENANCE.md).
...
remain behind a login. See [`data/national/PROVENANCE.md`](../data/national/PROVENANCE.md).
